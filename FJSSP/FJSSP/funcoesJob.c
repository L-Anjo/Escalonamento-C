 ...
 * @author Luis Anjo
 * @date 2022
 * @brief arvore Binaria Genérica
 *
 * Metodos para manipular uma arvore Binaria Genérica
 * @bug No known bugs.
*/
...
/**
 * @brief Adiciona Operacão ao Job
 *
 * @param Node - Nodo da arvore
 * @param ListaOperacoes - Operacão que sera adicionada
 * @param cod - Cod Job
 */
	...
	/**
	 * @brief Adiciona Operacão e Maquina ao Job
	 *
	 * @param Node - Nodo da arvore
	 * @param ListaOperacoes - Operacão que sera adicionada
	 * @param Maquinaa- Maquina que sera adicionada
	 * @param cod - Cod Job
	 * @param codop - Cod Operacão
	 */
	Node* InsereMaqOpeJob(Node* d, ListaOperacoes* oper,Maquinaa* m,int codj, int codop) {
		...
		if (op) { //Se Existe Operacão com o respetivo codigo
			op = InsereMaquinaOperacao(op, m, codop);
		}
		else { //Cria Operacão e Insere Maquina
		...
		if (no == NULL) { //Se arvore não Existe Cria e Adiciona Operacão
...
/**
 * @brief Mostra Operacoes de uma única Job
 *
 * @param Node - arvore
 ...
 */
	Node* ApagaNode(Node* root) {
		...
		if (root->esquerdo == NULL && root->direito == NULL) {	//único nodo
			...
			if (root->esquerdo == NULL) {		//so tem sub-arvore direita...ligar à direita
				aux = root;
				root = root->direito;
				DestroyNode(&aux);
			}
...
/**
 * @brief Remove Operacão de uma Job
 *
 * @param Node - arvore 
 * @param codop - Codigo da Operacão a ser Apagada
